...
abstract class Kernel extends QMatrix {
	...
	//加入cosin距离，作为权重的方式加入，看看效果怎么样。
    static double dot2(svm_node[] x, svm_node[] y)
	{
		double lamba=Lamba.lamba;
		double lamba_for_cosin=Lamba.lamba_for_cosin;
		double sum=0;
		int xlen = x.length;
		int ylen = y.length;
		double x_index[]=new double[xlen];
		double y_index[]=new double[ylen];
		...
		//计算两个样本之间的edit-distance
		Edit_Distance distance=new Edit_Distance();
		sum=distance.edit_distance(x_index, y_index);
		sum=Math.exp(-1*lamba*sum);
		//计算两个样本之间的cosin距离
		double comsum=0;
		for(int i=0;i<x.length;i++)
		{
		...
		comsum=lamba_for_cosin*comsum/(x.length*y.length);
		return sum+comsum;
	}
    static double dot3(svm_node[] x, svm_node[] y)
    {
    	double sum=0;
    	//***********************************************/////
    	int index=0;
    	ArrayList<svm_node>node_x_left=new ArrayList<svm_node>();
    	ArrayList<svm_node>node_x_right=new ArrayList<svm_node>();
    	while(x[index].value!=2)
    	{
    		node_x_left.add(x[index]);
    	}
    	node_x_left.add(x[index]);
    	index++;
    	while(index<x.length)
    	{
    		node_x_right.add(x[index]);
    		index++;
    	}
    	//***********************************************/////
    	index=0;
    	ArrayList<svm_node>node_y_left=new ArrayList<svm_node>();
    	ArrayList<svm_node>node_y_right=new ArrayList<svm_node>();
    	while(y[index].value!=2)
    	{
    		node_y_left.add(y[index]);
    	}
    	node_y_left.add(y[index]);
    	index++;
    	while(index<y.length)
    	{
    		node_y_right.add(y[index]);
    		index++;
    	}
    	//.........................................../////
    	double x_left[]=new double[node_x_left.size()];
    	double x_right[]=new double[node_x_right.size()];
    	double y_left[]=new double[node_y_left.size()];
    	double y_right[]=new double[node_y_right.size()];
    	
    	for(int i=0;i<x_left.length;i++)
    	{
    		x_left[i]=node_x_left.get(i).value;
    	}
    	for(int i=0;i<x_right.length;i++)
    	{
    		x_right[i]=node_x_right.get(i).value;
    	}
    	for(int i=0;i<y_left.length;i++)
    	{
    		y_left[i]=node_y_left.get(i).value;
    	}
    	for(int i=0;i<y_right.length;i++)
    	{
    		y_right[i]=node_y_right.get(i).value;
    	}
    	//calculate edit distance
    	Edit_Distance distance=new Edit_Distance();
    	double sum1=distance.edit_distance(x_left, y_left);
    	double sum2=distance.edit_distance(x_right, y_right);
    	sum=Math.exp(-1*Lamba.lamba*sum1)+Math.exp(-1*Lamba.lamba_for_structure*sum2);
    	return sum;
    }
